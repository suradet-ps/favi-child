...
FaviChild reads as a quietly-confident clinical tool brand that believes in editorial calm over dense dashboard darkness. The base canvas is **warm cream** (`{colors.canvas}` — #f7f7f4) holding warm near-black ink (`{colors.ink}` — #26251e) for body and display alike. The single brand voltage is **FaviChild Orange** (`{colors.primary}` — #f54e00) reserved for primary CTAs and the wordmark — used scarcely.

Type runs **Inter** as the single sans family. Display sits at weight 400 with negative letter-spacing — a magazine-editorial voice rather than tech-bombastic. JetBrains Mono carries every code surface (and numeric surfaces are roughly half the page).
...
- Single CTA color: `{colors.primary}` (FaviChild Orange #f54e00). Used scarcely.
...
### Brand & Accent
- **FaviChild Orange** (`{colors.primary}` — #f54e00): Primary CTA pills, wordmark, hero accent. Used scarcely.
- **FaviChild Orange Active** (`{colors.primary-active}` — #d04200): Press state.
...
- **On Primary** (`{colors.on-primary}` — #ffffff): White text on FaviChild Orange.
...
### Font Family
**Inter** is the display + body family. Fallback: `system-ui, "Helvetica Neue", Helvetica, Arial, sans-serif`. Code surfaces switch to **JetBrains Mono**.
...
Inter is the primary family (no licensed typeface dependency). **GT Sectra** is an alternative for a more editorial feel.
...
### Decorative Depth
- **IDE-mockup cards** are the only "elevated" element. White card on cream canvas with internal pane structure mimicking the actual product editor.
...
**`top-nav`** — Background `{colors.canvas}`, text `{colors.ink}`, height 64px. Layout: FaviChild wordmark left, primary horizontal menu (Pricing / Features / Enterprise / Blog / Forum / Careers), Sign In + Download primary CTA right.

### Buttons

**`button-primary`** — The signature FaviChild Orange CTA. Background `{colors.primary}`, text `{colors.on-primary}`, type `{typography.button}` (14px / 500), padding 10px × 18px, height 40px, rounded `{rounded.md}` (8px).
...
**`comparison-card`** — Side-by-side "FaviChild vs other dosing tools" card. Same surface and rounding; internally split into 2 columns.
...
**`cta-band`** — Pre-footer "Try FaviChild now" band. Background `{colors.canvas}`, centered display headline in `{typography.display-lg}`, single FaviChild Orange CTA. 96px vertical padding.
...
### Do
- Reserve `{colors.primary}` (FaviChild Orange) for primary CTAs and brand wordmark.
...
### Don't
- Don't introduce a secondary brand action color. FaviChild Orange is the only one.
...
7. FaviChild Orange stays scarce.
8. Timeline pastels stay scoped to in-product agent visualizations.

## Known Gaps

- Inter is the primary family (no licensed typeface dependency).
